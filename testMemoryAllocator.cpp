...
//	jj01::test_primitives();    //怎么使用分配器分配内存
...
//	jj331::test_bitmap_allocators();
    ...
    //测试cookie
    //以下將 int 改為 double 結果不變，意味上述 ints 間隔 8 (而非 4) 乃是因為 alignment.
    jj33::cookie_test(std::allocator<double>(), 1);				//相距 10h (表示帶 cookie)
    jj33::cookie_test(__gnu_cxx::malloc_allocator<double>(), 1);  //相距 10h (表示帶 cookie)
    jj33::cookie_test(__gnu_cxx::__pool_alloc<double>(), 1);		//相距 08h (表示不帶 cookie)
